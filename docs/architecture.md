...
- ~~Brisca fijo a 2 plazas en el lobby pese a que el motor soporta 2-4~~ — resuelto post-roadmap (ver sección final del documento).
...
**Roadmap M0-M7 completo.** Próximos pasos posibles, todos fuera del roadmap original y a decidir por el usuario: M6 parte 2 (OAuth + despliegue a producción, diferida por falta de credenciales), más juegos, o pulir deuda técnica acumulada.

## Post-roadmap — Torneos suizos + Brisca con aforo variable (implementado 2026-07-06)

Dos fast-follows pedidos explícitamente por el usuario tras cerrar M0-M7: el formato suizo que M5 había diferido a propósito, y activar el aforo 2-4 de Brisca que quedaba como deuda documentada desde M4.

### Brisca 2-4 jugadores

Cambio de una sola línea en `packages/db/src/seed.ts` (`maxPlayers: 2 → 4`) — el motor ya soportaba 2-4 desde M4 (tests con 3 y 4 asientos) y `BriscaBoard.tsx` ya iteraba genéricamente sobre `view.points`/`view.currentTrick` sin nada fijo a 2. El selector de aforo variable en el lobby (`_app.juegos.$slug.tsx`, añadido en M7 para Pista Única) ya era genérico y se activó solo con el cambio de catálogo, sin tocar frontend. Verificado por lectura de código (motor + tablero ya genéricos, confirmados línea a línea) más la reutilización del mismo mecanismo de aforo variable ya verificado end-to-end con Pista Única — no se escribió un E2E dedicado para Brisca 3-4 jugadores por no sumar más registros al rate-limit de la suite (ver hallazgo operativo más abajo).

### Torneos suizos (`format: 'swiss'` junto a `'single_elim'`)

**Schema** (migración `0009_blushing_blockbuster.sql`): `'swiss'` añadido al enum `tournament_format`; `tournamentParticipants.points` (real, default 0) — solo lo usa suizo (1 punto por victoria o bye, 0.5 por empate, acumulado ronda a ronda); en eliminación se queda en 0 sin usarse, ya que ahí `finalPlacement` decide el avance.

**Motor puro nuevo** (`apps/server/src/tournaments/swiss.ts`, mismo estilo sin IO que `bracket.ts`): `swissRoundsForSize(n)` = `ceil(log2(n))`. `pairSwissRound(standings, playedPairs, alreadyHadBye)`: ordena por puntos desc (desempate por seed asc), separa al bye si el número es impar (recae en el peor clasificado que todavía no haya descansado — si ya todos descansaron, cae en el último de todas formas en vez de colgarse) y empareja de arriba abajo evitando repetir una pareja ya jugada cuando hay alternativa en el mismo grupo de puntos (si no la hay, permite la revancha como último recurso antes que dejar a alguien sin partida). 10 tests cubren emparejamiento sin historial, evitar revancha, revancha forzosa, bye simple, bye rotado y el caso límite de "todos ya descansaron".

**Runner** (`apps/server/src/tournaments/service.ts`, refactor): `generateRound` se mantuvo **sin cambios y 100% reutilizado** entre formatos (crea parejas o byes de forma agnóstica); todo lo demás se separó en un par de funciones por formato (`onEliminationMatchFinished`/`onSwissMatchFinished`, `advanceEliminationRound`/`advanceSwissRound`, `finishEliminationTournament`/`finishSwissTournament`), con `onMatchFinishedHook` y `recoverOnBoot` decidiendo cuál llamar según `tournament.format`. Diferencia clave de reglas: eliminación fuerza un ganador incluso en empate (mejor sembrado, para poder avanzar el bracket); suizo admite un empate real (0.5 puntos cada uno, `winnerUserId` queda `null`) porque no necesita resolver un ganador único por partida, solo acumular puntos. Clasificación final del suizo: puntos desc, empates comparten puesto (misma convención "1-2-2-4" que ya usa `PlayerRank`), desempate estable por seed.

**tRPC + frontend**: `tournaments.create` gana `format` (enum, default `single_elim`); `getById` expone `format` y `points` por participante. `_app.torneos.index.tsx` añade el selector de formato al crear. `_app.torneos.$id.tsx`: `Bracket` (columnas de rondas/partidas) se reutilizó **sin ningún cambio** para suizo — ya era agnóstico a la geometría del torneo, solo pinta lo que hay en cada ronda; se añadió un aviso de "Empate" en `MatchCard` para el único caso que antes no podía ocurrir (`winnerUserId` nulo con la partida terminada); y una `Standings` en vivo (ordenada por puntos, visible durante `running` solo para suizo) además de la ya existente al terminar.

**Bug real encontrado y arreglado de paso (no de esta feature, preexistía)**: `CreateTournamentForm` inicializaba `gameId` con `useState(activeGames[0]?.slug ?? '')` — como `games.list` es asíncrono, en el primer render `activeGames` está vacío y ese `useState` (que solo se evalúa una vez) deja `gameId` en `''` para siempre; el `<select>` del navegador sí muestra el primer juego por defecto (comportamiento nativo de fallback), pero el estado de React nunca se sincroniza, así que el botón "Crear" quedaba deshabilitado sin ninguna pista visual de por qué — solo se salía del atolladero si el usuario tocaba manualmente el desplegable. Arreglado con un `useEffect` que sincroniza `gameId` en cuanto llegan los juegos. Este bug llevaba latente desde M5; lo expuso el primer intento de E2E de creación de torneos (nunca se había automatizado antes, los firetests de M5 se hicieron a mano).

**Verificación**: `pnpm turbo lint typecheck test build` en verde (23/23, 10 tests nuevos). E2E nuevo `e2e/tests/tournament-swiss.spec.ts` (4 `BrowserContext` reales): torneo suizo de 4 jugadores completo — ronda 1 sin bye (par), reemparejamiento real por puntos para la ronda 2, cierre automático a las 2 rondas (`swissRoundsForSize(4)=2`) con clasificación final. `e2e/tests/tournament-elim.spec.ts` nuevo como regresión explícita del refactor de `service.ts` (2 jugadores, eliminación directa, sigue funcionando igual que en M5). Ambos usan una estrategia de juego determinista para tres en raya (rellenar siempre la primera casilla vacía en tu turno, sin depender de qué asiento te toque) que gana en el 7º movimiento por diagonal — reutilizable para futuros E2E de torneos.

**Hallazgo de testing reutilizable**: el banner visual "Te toca a ti" y la región `aria-live` (`role="status"`, `sr-only`) contienen el mismo texto — un `getByText('Te toca a ti')` sin acotar a `<p>` choca en modo estricto contra los dos elementos y, si el error se captura en silencio (`.catch(() => false)`), el síntoma es que el turno parece no detectarse nunca, sin ningún error explícito. `match.spec.ts` ya documentaba este mismo problema para el texto de resultado; ahora también aplica a "Te toca a ti"/"Turno del rival" en cualquier E2E que necesite detectar de quién es el turno.

**Hallazgo operativo (acumulativo con M6/M7)**: con 9 specs E2E, la suite ya no cabe ni en dos tandas bajo el rate-limit de `/api/auth/register` (8/15 min) — hoy suma 17 registros repartidos en 3 tandas. Se verificó en 3 tandas con reinicio del servidor entre cada una. Sigue sin tocarse el rate-limit de producción; la mejora de fondo pendiente (no aplicada) sería un pool de cuentas de prueba reutilizables por login en vez de registro nuevo por spec.

## Prototipo visual 3D — Conecta 4 con Three.js (implementado 2026-07-06)

El usuario pidió dar un aspecto profesional a los tableros con Three.js y animaciones de movimiento. Dado el tamaño del cambio (5 juegos con geometría e interacción distintas), se acordó explícitamente con el usuario: **un solo juego primero como muestra** (Conecta 4, elegido porque la caída por gravedad y el marco físico del juego real encajan de forma natural en 3D) y **capa accesible oculta** superpuesta al 3D en vez de sacrificar la navegación por teclado/lectores de pantalla añadida en M6.

**Dependencias nuevas** (`apps/web`): `three`, `@react-three/fiber` (React ↔ Three.js), `@react-three/drei` (helpers: `OrthographicCamera`, `RoundedBox`, `ContactShadows`), `@react-spring/three` (animación por muelle/física, no por keyframes).

**Escena** (`apps/web/src/games/ConnectFourBoard.tsx`, reescrito por completo): marco redondeado azul oscuro con 42 huecos circulares recesados; fichas como cilindros rotados 90° (un cilindro tiene el eje por defecto en Y — sin rotar se ve de canto, como una barra fina, no como una moneda) con material `meshStandardMaterial` (algo de rugosidad, casi sin metalicidad, para un acabado "plástico de juguete"); iluminación de dos direccionales + ambiental y `ContactShadows` para que las fichas proyecten sombra real sobre el tablero. Colores de ficha fijos (no ligados a los tokens `tb-` de tema claro/oscuro) — una ficha física no cambia de color con la luz de la habitación.

**Cámara**: ortográfica, fija, mirando de frente (sin `OrbitControls`) — decisión deliberada para que la capa accesible (ver abajo) sea una simple rejilla CSS sin ningún cálculo de proyección 3D→2D. Frustum definido con `left/right/top/bottom` explícitos en unidades de mundo (con `manual` para que R3F no los recalcule al redimensionar) en vez de `zoom` atado a píxeles — un primer intento con `zoom` numérico cortaba la fila inferior del tablero porque el tamaño en píxeles del lienzo no tiene por qué coincidir con el aspecto real del tablero en unidades de mundo. El contenedor CSS usa `aspectRatio` calculado a partir de las mismas constantes `BOARD_W`/`BOARD_H` del frustum, para que nunca se desincronicen.

**Animación de caída**: al detectar (comparando el tablero contra la ronda anterior de `seq`) qué celda es la recién ocupada, esa ficha —y solo esa— arranca un `useSpring` de `@react-spring/three` desde por encima del tablero hasta su fila de destino, con una configuración de muelle con rebote (`tension:190, friction:13`) que se pasa de largo y asienta, imitando gravedad + rebote real sin física manual. El resto de fichas ya colocadas (incluidas las de una partida reanudada a mitad) se renderizan directamente en su posición final, sin repetir la caída — el valor `isNew` se congela en el montaje de cada `<Piece>` (una por celda, `key` estable) para que un cambio de turno posterior no reproduzca la animación de fichas ajenas.

**Interacción añadida**: al pasar el ratón (o el foco de teclado) por una columna jugable se muestra una ficha fantasma semitransparente del color del jugador activo flotando sobre esa columna — mismo patrón de "previsualización de jugada" que ya usan Reversi/Pista Única, aquí en 3D.

**Capa accesible** (superpuesta, no eliminada): los mismos 42 `<button>` con los mismos `aria-label` que la versión 2D anterior siguen ahí, ahora invisibles (`opacity-0`) pero perfectamente funcionales — clic y foco de teclado (`Tab`+`Enter`) disparan el mismo `handleDrop`, con un anillo de foco visible (`focus-visible:opacity-100` + `ring`) para quien navegue sin ratón. Como la cámara es fija, la rejilla CSS de botones coincide siempre con la escena 3D sin ningún ajuste manual de posición.

**Verificación**: `pnpm turbo lint typecheck test build` en verde (23/23). Verificado visualmente con capturas de Playwright contra una partida real por WebSocket (Chromium headless renderiza WebGL sin configuración adicional en este entorno) — tablero vacío, fichas asentadas con forma de disco correcta tras el arreglo de rotación, ficha fantasma al pasar el ratón. Verificado además que la capa accesible funciona de verdad por teclado: enfocar un botón invisible con foco programático y pulsar `Enter` incrementa el recuento de fichas en el tablero igual que un clic.

**Hallazgos de esta pasada (útiles para replicar el patrón en el resto de juegos)**:
- Un `cylinderGeometry` tiene el eje por defecto en Y — para que se vea como una ficha/moneda de frente hay que rotarlo `Math.PI/2` en X; sin la rotación se ve como una barra fina (el error visual más obvio de esta pasada, cazado por captura de pantalla, no por los tipos ni el build).
- `OrthographicCamera` con `zoom` numérico depende del tamaño en píxeles del lienzo, no de las unidades de mundo de la escena — para un encuadre que no se recorte ni se desincronice al redimensionar, hay que fijar el frustum explícito (`left/right/top/bottom` + `manual`) y hacer que el aspecto del contenedor CSS coincida exactamente con el aspecto mundo del tablero.
- El patrón de "congelar en el montaje si esta pieza nace animada" (`useState(isNewProp)`, sin volver a leer la prop después) evita que un cambio de turno posterior reproduzca por error la animación de fichas que ya estaban asentadas.
...
## Aviso de fin de partida como capa sobre el tablero + arreglo de caché al reentrar (2026-07-06)

Dos pulidos pedidos por el usuario, aplicados en el único punto compartido por los 5 juegos (`_app.partida.$id.tsx`, no hay nada que tocar por juego):

**Aviso de fin de partida**: antes el resultado ("¡Has ganado!"/"Has perdido"/"Empate") **sustituía** por completo al tablero. Ahora el tablero se queda montado (sus controles ya quedan inertes solos porque `activePlayers` vacío hace que `myTurn` sea `false` en todos los juegos) y el resultado aparece como una tarjeta centrada con `backdrop-blur-md` sobre un velo `bg-tb-surface/25` — se ve la posición final del tablero de fondo, ligeramente atenuada, con la tarjeta de resultado bien sólida y legible encima. Primer intento con blur sobre TODO el tablero (`backdrop-blur-sm` en el velo completo) volvía casi ilegibles algunas fichas/glifos fino — se movió el blur a la propia tarjeta (glassmorphism clásico: blur detrás de la tarjeta, no detrás de todo el fondo) y se aligeró el velo. Animación de entrada nueva y reutilizable `.tb-modal-in` en `app.css` (fade+scale sutil, respeta `prefers-reduced-motion`).

**Bug real: `matches.myActive` nunca se invalidaba al terminar una partida.** Repro exacta que dio el usuario: terminas una partida, no pulsas "volver al catálogo", navegas a otra sección por el menú, vuelves a la ficha del mismo juego → te lleva otra vez a la partida ya acabada; hay que salir y volver a entrar una segunda vez para que cargue el lobby de verdad. Causa: ninguna mutación ni efecto invalidaba la query `matches.myActive` cuando el WS entregaba `match.ended` — solo se invalidaba tras `leave`/`create`/`join` desde el propio lobby, nunca desde la página de partida. `_app.juegos.$slug.tsx` decide qué panel mostrar (crear mesa / tu mesa / reanudar) leyendo esa query cacheada por React Query (`staleTime` por defecto = 0, pero sin invalidar nadie dispara el refetch hasta el siguiente mount). Arreglo: `_app.partida.$id.tsx` invalida `utils.matches.myActive` en un `useEffect` en cuanto `ended` pasa a tener valor — así la próxima vez que se visite la ficha del juego, la query ya está marcada como inválida y se refresca en el propio mount, en vez de servir indefinidamente el `in_game` desactualizado de la partida que ya terminó.

**Verificación**: `pnpm turbo lint typecheck test build` en verde (23/23). Comprobado con Playwright en un flujo real: terminar una partida de tres en raya, capturar el overlay (tablero visible de fondo, tarjeta legible), navegar a `/salas` sin pulsar "volver al catálogo", volver a `/juegos/tres-en-raya` y confirmar en un único intento que se muestra el formulario de crear mesa, no la partida acabada. Suite completa de 10 specs E2E en verde en 3 tandas (sin regresiones en el resto de juegos, que comparten el mismo banner de resultado).

## Lobby de partida: dos columnas, "sin tiempo", mesas de amigos/públicas (implementado 2026-07-06)

Serie de pedidos encadenados sobre la experiencia de crear/unirse a una mesa, resueltos en varias pasadas cortas sobre los mismos archivos (`_app.juegos.$slug.tsx`, `_app.salas.tsx`, `_app.tsx`, `_app.sala.$code.tsx`).

**Fix residual del bug anterior**: invalidar `matches.myActive` con `.invalidate()` no basta cuando nadie tiene esa query montada en ese momento (marca la query stale sin refrescarla de verdad) — `_app.partida.$id.tsx` ahora también limpia la caché a mano con `utils.matches.myActive.setData(undefined, prev => prev?.matchId === matchId ? null : prev)` en cuanto `ended` tiene valor, sin esperar al refetch del siguiente mount.

**Rediseño en dos columnas + "sin tiempo" (amistosa) + cancelación de mesa**: `GameLobby` pasa de una pila vertical (mesas esperando → formulario) a dos columnas — izquierda: mesas esperando jugadores o la propia una vez creada; derecha: configuración con "Comenzar" al fondo, que al pulsarse bloquea los controles (`pointer-events-none`, resumen real de lo elegido) y hace que la izquierda muestre `YourWaitingTable` (código, asientos, abandonar) en vez de la lista pública; al completarse la mesa, navega sola a `/sala/$code` sin duplicar el ready-check. Se retira el checkbox "Partida clasificatoria": la partida es competitiva siempre salvo que se elija el chip nuevo "Sin tiempo", que la vuelve amistosa (`rated = turnDurationS !== null`, decidido en servidor). "Sin tiempo" es soporte real del motor, no una duración larga disfrazada — `matches.turnDurationS`/`MatchRuntime.turnDurationS` pasan a `number | null`, y `lifecycle.ts` solo arma el temporizador de turno si no es `null`.

**Anfitrión abandona la mesa en espera → se cancela para todos** (antes solo borraba su propio asiento, dejando mesas fantasma con 0 jugadores en `waiting` para siempre — se encontraron y limpiaron varias en la BD de desarrollo). `matches.leave` distingue si quien se va es `hostUserId`: si lo es, la mesa pasa a `state:'cancelled'` (enum y broadcast `match.lobby` ya lo contemplaban desde M2 sin que nada lo disparase). Cliente: `YourWaitingTable` y `/sala/$code` reaccionan en vivo con un aviso y vuelta al catálogo.

**Toque visual: se probó Three.js, se revirtió por petición explícita del usuario**. Primera pasada: un par de dados 3D (mismo patrón `@react-three/fiber`+`drei` del prototipo de Conecta 4) como cabecera del panel de creación. El usuario lo rechazó ("no que añadas elementos 3D sin mucho sentido"), pidiendo pulir los controles reales en su lugar — se retiró `LobbyScene.tsx` por completo y se invirtió el esfuerzo en: `ModeCard`/`Chip` con elevación+sombra al hover y check animado al seleccionar, interruptor deslizante real (antes checkbox) para "Sala privada", el CTA "Comenzar" con la misma elevación de sombra que el resto de tarjetas, y la píldora de "Mesa abierta…" con punto pulsante en vez de caja discontinua apagada. Lección de proceso: en peticiones de "dale un toque más atractivo", confirmar antes de introducir una tecnología nueva y vistosa si lo pedido en realidad apunta a pulir interacción existente.

**Salas de amigos en el rail + salas públicas agrupadas por juego + código de sala oculto**: (1) `rail.friendsRooms` (antes basado en `presence==='in_game'`, que solo se activa cuando la partida ya arrancó) pasa a consultar un endpoint nuevo `matches.friendsWaiting` (mesas `waiting` cuyo anfitrión es amigo, **incluidas las privadas** — la visibilidad la da la amistad, no `isPrivate`) con botón "Unirse" al hover que navega directo a `/sala/$code`. (2) `matches.listPublic` filtra mesas con `0 < jugadores < aforo` (ni vacías —huérfanas de antes del fix de cancelación— ni llenas) y el cliente las agrupa por juego con cabecera+contador, mostrando solo juegos con alguna mesa unible. (3) Se oculta/elimina el código de mesa como concepto de UI (input "unirse por código", píldora+copiar en `YourWaitingTable`/`/sala/$code`) — con amigos y salas públicas descubribles con un clic, no hace falta compartir/teclear código; la URL interna sigue usándolo como identificador (`/sala/$code`), solo deja de mostrarse. `matches.getByCode` gana `gameName` para que la cabecera de `/sala/$code` muestre el juego. Los 4 E2E que extraían el código de la UI para navegar directo se reescribieron para unirse pulsando "Unirse" desde la ficha del juego, como un usuario real.

Dos textos de copy ajustados de paso: "Solo se entra con el código de la mesa" → "Solo tus amigos podrán unirse"; "comparte el código o espera…" → "espera a que alguien se una desde la ficha del juego".

**Verificación**: `pnpm typecheck`/`lint`/`test` en verde en cada pasada; E2E ad-hoc con Playwright por pieza (2 columnas+sin tiempo+comenzar bloqueado; anfitrión abandona→mesa desaparece; sala de amigos→unirse desde el rail; agrupación por juego excluyendo llenas/vacías), capturas revisadas visualmente antes de dar cada pieza por buena. Suite E2E existente migrada al flujo de clic y verificada en verde salvo una repetición bloqueada por el rate-limit de registro de la propia sesión de pruebas.

## Sistema de reputación de usuarios (implementado 2026-07-06)

Nuevo eje de comportamiento (1-100, 100=intachable), explícitamente independiente del rating de nivel de juego (Glicko-2, M4): visible al pasar el ratón sobre cualquier jugador, se pierde por abandonar partidas/reportes fundados/lenguaje ofensivo y se recupera jugando limpio con el tiempo. Acordado con el usuario de antemano: sin consecuencias funcionales todavía (solo informativo/social), insultos bloquean el envío (aviso la 1ª vez, penalización desde la 2ª), reportar solo tras compartir partida, abandono mutuo neutral.

### Esquema y puntuación

`users.reputation` (smallint 1-100, default 100, `check` en BD). Ledger append-only `reputationEvents` (mismo patrón que `ratingHistory` de M4: cada evento con before/after, no solo un contador) con `reason` enum (`match_abandoned`/`match_forfeit_timeout`/`match_completed_clean`/`chat_blocked_profanity`/`user_report`/`passive_recovery`). Tabla `userReports` (`unique(reporterId, reportedUserId, matchId)` como guarda antiabuso).

Reglas puras (`apps/server/src/reputation/rules.ts`, sin IO, mismo reparto que `ratings/glicko.ts`+`ratings/service.ts` — 9 tests unitarios): abandono voluntario -8 (escalando +2 por cada abandono/timeout en 30 días, tope -20 — reincidencia pesa más que un desliz puntual), timeout -5 (misma escalada, señal más débil que un abandono explícito), partida terminada con normalidad +1 (tope 100, el incentivo de "buen juego"), insulto bloqueado 0 la 1ª vez en 24h / -3 desde la 2ª, reporte válido -5, recuperación pasiva +1 cada 3 días sin incidentes (job periódico, mismo patrón `setInterval` que `ws/heartbeat.ts`).

### Abandono unilateral y mutuo — completando algo que el esquema ya anticipaba

`matches.state` ya incluía `'abandoned'` y `matchPlayers.leftAt` ya existía desde M2, sin ningún camino de código que los usara — esta pasada los pone en marcha. Abandono unilateral (`match.forfeit`, WS nuevo): reutiliza `forfeitMatch` (ya existente desde M2 para el forfeit por timeout) con un parámetro nuevo `reason: 'timeout' | 'quit'`, aplicando al cerrar la reputación correspondiente al asiento que abandona y +1 "clean" al resto. Abandono mutuo (`match.abandonRequest`/`match.abandonCancel`, nuevos): un `Set<number>` de asientos que lo han pedido vive en `MatchRuntime` (transitorio, como `readyCheckEndsAt`); al coincidir con todos los asientos sentados, la partida pasa a `state:'abandoned'`, `leftAt` para todos, **sin** tocar rating/stats/reputación de nadie, con un `reason:'abandoned'` nuevo en `match.ended` — deliberadamente no se llama a `emitMatchFinished` (el listener de torneos no tendría ranking con qué avanzar el bracket; las mesas de torneo quedan fuera de alcance de este cierre). Cualquier movimiento real cancela las peticiones pendientes de todos.

### Filtro de lenguaje y reportes

`apps/server/src/moderation/profanity.ts`: lista propia ES+EN (sin dependencia externa, coherente con el resto del proyecto) con normalización (minúsculas, sin acentos, leetspeak básico) y límites de palabra. Enganchado en `MatchService.handleChat` y `SocialService.sendDirectMessage` (chat de mesa y DMs comparten filtro): si detecta, no persiste el mensaje y responde al emisor reutilizando el `match.error` ya existente. Reportar (`moderation.report`, router nuevo): valida que reportador y reportado compartieran de verdad una partida antes de aceptar el reporte.

### Perfil público y tarjeta al pasar el ratón

Router nuevo `users.publicProfile` (nick, fecha de alta, reputación, partidas/victorias agregadas). `UserHoverCard.tsx` (nuevo, sin librería de tooltip/popover — no había ninguna en el proyecto): envuelve el avatar en cualquier sitio donde ya se muestra gente, dispara la query solo al primer hover (`enabled: open`) y pinta nick, "Miembro desde…", barra de reputación (verde/ámbar/rojo por tramo) y stats. Integrado en `FriendRow`, los asientos de mesa/sala, la fila de jugadores de la partida y `/rankings` — un único componente reutilizado en los 4 sitios.

### Verificación

9 tests unitarios nuevos de las reglas de puntuación (escalada, clamp 1-100, ventana de insultos) — 43 en total en `apps/server`, todos en verde junto a `pnpm typecheck`/`lint` de todo el monorepo. Dos E2E ad-hoc con Playwright, dos navegadores reales cada uno: (1) A abandona voluntariamente a mitad de partida → pierde, reputación 100→92; B lo reporta desde el overlay de fin de partida → 92→87; segunda partida con abandono mutuo aceptado por ambos → ni rating ni reputación de nadie se tocan; (2) insulto en el chat de mesa → se bloquea sin llegar al otro jugador, mensaje normal sí llega; hover sobre el chip de un jugador en la partida muestra su tarjeta con reputación 100/100 y el nick correcto.
