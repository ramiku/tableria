...
export const Route = createFileRoute('/_app/social/mensajes/$conversationId')({ component: ThreadPage });
...
function ThreadPage() {
  ...
  const display = getConversationDisplay(conversation, t('messages.unknownUser'));
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 border-b border-tb-border px-4 py-3">
        ...
        <Avatar initial={display.initial} color={display.color} size={36} />
      ...
      {display.kind === 'group' && (
        <VoiceCallBar room={{ kind: 'conversation', conversationId }} label={display.name} meId={me.id} />
      )}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
