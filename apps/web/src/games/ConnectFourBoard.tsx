...
import { CONNECT_FOUR_BOARD_PRESETS, CONNECT_FOUR_DEFAULT_PRESET, type ConnectFourView } from '@tableria/games';
...
const DEFAULT_SIZE = CONNECT_FOUR_BOARD_PRESETS[CONNECT_FOUR_DEFAULT_PRESET];

function xFor(col: number, cols: number): number {
  return (col - (cols - 1) / 2) * CELL;
}
function yFor(row: number, rows: number): number {
  return ((rows - 1) / 2 - row) * CELL;
}

function Piece({ col, row, cols, rows, seat, isNew }: { col: number; row: number; cols: number; rows: number; seat: 0 | 1; isNew: boolean }) {
  ...
  const restY = yFor(row, rows);
  const { y } = useSpring({
    from: { y: dropsFromAbove ? yFor(-2, rows) : restY },
    to: { y: restY },
  ...
  return (
    ...
    <animated.mesh position-x={xFor(col, cols)} position-y={y} position-z={0.16} rotation-x={Math.PI / 2} castShadow receiveShadow>
...
function GhostPiece({ column, cols, rows, seat }: { column: number; cols: number; rows: number; seat: 0 | 1 }) {
  return (
    <mesh position={[xFor(column, cols), yFor(-1, rows), 0.16]} rotation-x={Math.PI / 2}>
      <cylinderGeometry args={[CELL * 0.42, CELL * 0.42, CELL * 0.22, 32]} />
...
function Frame({ cols, rows, boardW, boardH, hoverColumn }: { cols: number; rows: number; boardW: number; boardH: number; hoverColumn: number | null }) {
  return (
    <>
      <RoundedBox args={[boardW, boardH, 0.4]} radius={0.08} position={[0, 0, -0.06]} receiveShadow castShadow>
      ...
      {Array.from({ length: cols * rows }, (_, i) => {
        ...
        return (
          <mesh key={i} position={[xFor(col, cols), yFor(row, rows), 0.15]}>
...
export function ConnectFourBoard({ matchId, seq, mySeat, myTurn, view: rawView }: BoardProps) {
  ...
  const board = view?.board ?? Array(rows * cols).fill(null);
      ...
      >
        <Canvas shadows dpr={[1, 2]} className="!absolute inset-0">
          <OrthographicCamera
            ...
            manual
            position={[0, 0, 10]}
            left={(-boardW / 2) * CAMERA_PAD}
            right={(boardW / 2) * CAMERA_PAD}
            top={(boardH / 2) * CAMERA_PAD}
            bottom={(-boardH / 2) * CAMERA_PAD}
            near={0.1}
            far={100}
          />
          ...
          <Frame cols={cols} rows={rows} boardW={boardW} boardH={boardH} hoverColumn={myTurn ? hoverColumn : null} />
          {board.map((cell, i) => {
            if (cell === null) return null;
            const col = i % cols;
            const row = Math.floor(i / cols);
            return <Piece key={i} col={col} row={row} cols={cols} rows={rows} seat={cell} isNew={i === newIndex} />;
          })}
          {canGhost && <GhostPiece column={hoverColumn} cols={cols} rows={rows} seat={mySeat as 0 | 1} />}
